...
- Rows appear only when they apply to your Mac: Automation lists the terminals you actually have installed, Accessibility appears while an editor whose windows Toki raises is running, Local Network while Remote Control is on. A permission you refused earlier is marked as refused and offers System Settings instead of an **Allow** button macOS would ignore, since it never asks twice.
