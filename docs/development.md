...
When Toki was installed by a cask, the in-app updater detects it (`BrewCask.installedCask` — the Caskroom entry is a symlink to the installed bundle) and routes "Install update" through `brew upgrade --cask aashutoshrathi/tap/<toki|toki-beta>` instead of swapping the DMG underneath brew. This keeps brew's receipt in sync with the app on disk; the DMG path would desync it, and the next `brew upgrade` would then clobber the newer build with the cask's older one. Dev builds and manual installs keep the direct DMG path.

Naming the tap is load-bearing, not decoration. Homebrew refreshes a third-party tap before `install`/`upgrade` only every 24 hours unless the argument is an `owner/tap/cask`, which drops that window to five minutes (`Library/Homebrew/utils/auto-update.sh`). With a bare `toki`, an upgrade run in the first day of a release can resolve against a snapshot taken before the release existed, warn `Not upgrading toki, the latest version is already installed`, and exit 0 — a handoff that succeeds and changes nothing, with the app still offering the update. The updater also treats that outcome as a failure: it runs `brew update` and retries the upgrade once before reporting, and what it reports carries brew's own last line plus `brew update && brew reinstall --cask ...`, since repeating the upgrade only reproduces the no-op.
...
- **`brew finished but Toki is still <version>`** — brew ran without replacing the app. The message carries brew's own last line; `brew update && brew reinstall --cask aashutoshrathi/tap/toki` (or `toki-beta`) is the way out, because it refreshes the tap and ignores a receipt that already claims the new version. Check the tap carries the version the app offered before digging further.
