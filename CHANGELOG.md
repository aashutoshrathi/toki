...
- **The permissions Toki needs are a checklist now, not a series of surprises.** Every macOS permission Toki wants is one list, during onboarding and permanently under **Settings › Permissions**, saying what each is for and where it stands. Nothing is requested until you press its button, and every status is read with a check that cannot prompt.
...
- **The companion app answers multi-question, multi-select prompts.** When an agent asks through OpenCode's `question` tool or Claude's AskUserQuestion, with several questions each taking one answer or many, the phone now shows every question with its header, options, and descriptions: checkboxes where more than one applies, a single choice where one does. A lone single-choice question still answers on one tap; anything richer keeps the panel open while you toggle, then a **Submit** button turns your picks into the exact keypresses that picker needs, driving its tabs and confirming for you. Before this, a multi-select was mistaken for a plain approval and could not be answered from your phone at all.
- **Send an image from your phone.** A reply can now carry a picture: pick one from your library, take it with the camera, or paste it into the composer. Toki saves the image on your Mac and references its path in the message, so an agent that reads images picks it up and one that does not just says so. The picture goes to your Mac only, like everything else Remote Control sends, and only real image files are accepted.
- **The browser tab names the Mac and the chat.** The companion app's page title is now `<Mac name> - <chat title>`, so with tabs open on more than one Mac, or more than one conversation, you can tell them apart from the tab strip alone. It follows the privacy toggle: with names hidden, the chat half is masked, and it resets to Toki Remote Control when nothing is connected.
- **Leaving a paired session asks first.** The companion app's Home button dropped the pairing on a single tap, which was easy to hit by accident and meant re-pairing with Toki's code or QR. It now confirms before disconnecting; the connect screen's Home, with no live session to lose, still goes straight back.

### Changed

- **A fresh install no longer raises the Keychain dialog just because you opened the menu.** Reading Claude Code's sign-in is what put that dialog up, so it now waits for the checklist's Keychain row. A file-based sign-in is still found either way.
- **Toki turns on Tailscale HTTPS instead of telling you it is off.** Choosing Tailscale and starting the server now runs `tailscale serve` once per start rather than reporting the problem and waiting for a click. A handler already serving :443 is left alone.

### Fixed

- **The companion app keeps its session when you close the browser.** The phone kept its half of the session in tab-scoped storage, so closing the browser or iOS discarding the tab ended it; it now lasts as long as the Mac says. A 403 no longer ends a session by itself.
...
- A refusal from `tailscale serve` is named along with its fix: not being the tailnet operator, a tailnet without HTTPS certificates, and a signed-out Tailscale each come back with the command or sign-in that clears it, with a copy button instead of raw stderr.
