...
- Scanning Toki's Connect QR from inside the companion app works again on Tailscale. The scanner only accepted a code that pointed at the exact page you were already on, so scanning a direct Tailscale link (`https://<your-machine>.ts.net/?token=…`) on the hosted Toki RC interface — or a Toki RC link on the Mac's own page — was refused as not being a link for that page, even though both name the same Mac and the manual host-and-token form accepted them. The scanner now reads the host and token out of either link and connects. A local-network link still can't be used from the hosted interface (an HTTPS page cannot call a plain-HTTP address), but it now says which address it couldn't reach instead of calling the code invalid.
