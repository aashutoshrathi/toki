...
- One-click HTTPS setup: when `tailscale serve` isn't running, an "Enable HTTPS access" button starts it for you and re-checks reachability. Choosing Tailscale for "From anywhere" now also runs `tailscale serve` automatically once the server is up, so most setups need no button press at all.
