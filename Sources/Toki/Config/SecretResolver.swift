...
enum SecretResolver {
    ...
    static func runShell(_ command: String, timeout: TimeInterval = 15) throws -> String {
        ...
        let outputPipe = Pipe()
        let errorPipe = Pipe()
        // The login shell (-l) resolves user-managed PATH entries, but login profiles are
        // allowed to print, and anything they emit lands ahead of the command's own output -
        // a Keychain credential read comes back as "banner text\n{json}" and fails parsing.
        // A sentinel printed after the profiles have run marks where real output begins;
        // everything before it is discarded, on stderr too so a profile warning can't
        // garble a surfaced error.
