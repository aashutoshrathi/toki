...
- Spend Analytics gained a daily-usage heatmap, filterable by provider. Each cell is a day, shaded by the deepest the quota got that day - peak consumption rather than the last reading, so a mid-day reset doesn't erase a heavy morning. The scale is a single hue running light to dark, with separate steps chosen for dark mode rather than an inverted copy of the light ones, and "no data" is a neutral grey rather than a pale tint so it can't be misread as a very light usage day. Hovering a day breaks it down by account, heaviest first, with the number of readings taken. The scale was validated rather than eyeballed: every adjacent pair of steps clears the normal-vision separation floor and holds above it under all three common forms of colour blindness, and cell borders carry the palest steps, which fall below the 3:1 contrast bar on their own. The window is the shorter of 30 days and the configured history retention, so every cell shown covers a day actually retained.
