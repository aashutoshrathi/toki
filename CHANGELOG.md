...
- Spend Analytics gained a daily-usage heatmap, filterable by provider. Each cell is a day, shaded by the deepest the quota got that day - peak consumption rather than the last reading, so a mid-day reset doesn't erase a heavy morning. The scale is a single hue running light to dark, with separate steps chosen for dark mode rather than an inverted copy of the light ones, and "no data" is a neutral grey rather than a pale tint so it can't be misread as a very light usage day. Exact figures are on hover and exposed to VoiceOver, so nothing depends on colour alone. The window is the shorter of 30 days and the configured history retention, so every cell shown covers a day actually retained.
