...
- **Switching update channels works again.** Homebrew now refuses to load a cask from a third-party tap until that exact cask is trusted, and trust is per cask: installing `toki` never trusted `toki-beta`, so every switch failed with "nothing changed". Toki trusts the one cask it is moving to before switching, and a switch that fails anyway now carries the reason brew gave.
- **The Switch button stays inside the account card.** An account with a switch target and more than one quota window had a header row wider than the panel, so the button was sliced off at the right edge. Switch is an icon button carrying its name in the tooltip now, the shape the action already has elsewhere, and what little the row still has to give comes off the account name rather than the button.
- **Toki notices when Sarvam Code is waiting on an answer.** The app read pending questions for Claude and OpenCode only, so a Sarvam agent parked on a question looked busy rather than blocked in the agents list. It reads Sarvam's own session now, the same `request_user_input` call the companion app already understood.
- **A Zed thread started after Toki is running now shows up.** Zed commits its threads through a write-ahead log, which leaves the database file Toki keyed its cache on untouched, so the first read of the app's life was served forever: a thread opened later never appeared, and one that was live at launch went stale and dropped off. A failed read is no longer remembered as "no threads" either.
...
- **Answer agents running in Ghostty from your phone.** Replies, picker keys and terminal mirroring now route to a bare Ghostty session by its exact tty, so an agent started there is as reachable as one in tmux, iTerm or Terminal. Cursor and VS Code integrated terminals stay writable through the same path.
