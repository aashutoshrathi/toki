...
- Experimental "live in the notch" mode moves the status readout into the display notch instead of the menu bar. The notch is not an API surface - it is a region the menu bar draws around - so this places a borderless window over the gap between the two auxiliary menu bar areas macOS reports. The toggle only appears on hardware that actually has a notch, and if the geometry is unavailable it falls back to the menu bar rather than leaving nothing visible.
...
- Spend Analytics gained a daily-usage heatmap, filterable by provider. Each cell is a day, shaded by how much quota was actually consumed that day - measured as the total drop in remaining quota across the day's readings, not the standing remaining figure. Reading the standing figure would conflate state with activity: an account sitting at 0% remaining reports "fully used" every day until its quota resets, so idle days would paint as saturated. Quota resets raise the remaining figure and are ignored rather than counted as negative usage. The scale is a single hue running light to dark, with separate steps chosen for dark mode rather than an inverted copy of the light ones, and "no data" is a neutral grey rather than a pale tint so it can't be misread as a very light usage day. Hovering a day breaks it down by account, heaviest first, with the number of readings taken. The scale was validated rather than eyeballed: every adjacent pair of steps clears the normal-vision separation floor and holds above it under all three common forms of colour blindness, and cell borders carry the palest steps, which fall below the 3:1 contrast bar on their own. The window is the shorter of 30 days and the configured history retention, so every cell shown covers a day actually retained.
