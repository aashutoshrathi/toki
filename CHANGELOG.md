...
- **Quota on your phone.** Remote Control already runs a server and your phone already polls it, so usage no longer stays on the menu bar: the companion app shows a strip with the account that has least left, expanding to the full list on tap. It is the same reading the Mac renders, not a second source, so the two cannot disagree — and an account with no quota API sends no number at all rather than a bar drawn from a placeholder. A reading the Mac stopped refreshing says so instead of quietly looking current.
- **Tool calls in the companion app show that they finished.** Every tool result was already being sent to the phone and thrown away, so a call that returned a second ago looked exactly like one still running. Calls now show as in flight, then as succeeded or failed, with how long they took — and a second line saying what the call is actually doing, which the one-line summary hid whenever a command came with a description. Tool *output* deliberately stays on the Mac.
...
- **The companion app keeps its session when you close the browser.** Pairing grants a session of an hour to two days, but the phone kept its half in storage scoped to the tab — so closing the browser ended it, and so did iOS discarding a backgrounded tab, which happens within minutes of the phone locking. It now lasts exactly as long as the Mac says it does. A 403 also no longer ends a session by itself: the host setting answers with one for a network it was not meant to serve, so walking out of the house used to be a logout.
- **Toki no longer reports a working `tailscale serve` as missing.** Detection matched one literal spelling of the loopback address at one path, so `localhost`, an IPv6 loopback, or Toki served alongside anything else all read as "not serving". Worse, a status Toki could not read rendered identically to one that was genuinely empty — telling you HTTPS was off and offering to fix a setup that already worked. Those are now different answers, and the row says which.
- **`tailscale serve` starts itself on the machines that most need it to.** Auto-serve was gated on knowing the Mac's `.ts.net` name, which serving does not require — so on a Mac whose Tailscale ships no usable CLI, exactly where it cannot be read, the gate closed and serve silently never ran. It also says when it deliberately declines rather than leaving a warning that reads as though nothing happened. And where the name cannot be read, it is now taken from serve's own configuration instead of typed in by hand.
- **The permissions checklist reports what it actually checked.** A terminal that is not running makes macOS refuse to answer about Automation, and that was shown as "not granted yet" — so every closed terminal, including ones allowed long ago, sat there as a chore with an Allow button that could not produce a dialog. The Keychain row claimed success whenever the gate was open, even if the dialog had been denied, and offered no way to retry. **Send a test** logged a notification as delivered that macOS may never have shown; it now says it was handed over, and offers the Notifications settings pane, because going to look is the only way to know.
