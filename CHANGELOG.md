...
- Scanning Toki's Connect QR from inside the companion app works again, and two separate problems were behind it. A scanned link was applied by putting it in the page's address and reloading, but the scan button is only reachable from the "open a new link" screen, which clears the last saved connection first — so on an installed home-screen app, where a reload comes back at the app's start address without those details, the freshly scanned link was lost and you landed right back on the same screen. The link is now saved before the reload, so it survives either way; this also affected entering a host and token by hand. Separately, the scanner only accepted a code pointing at the exact page you were already on, so a direct Tailscale link (`https://<your-machine>.ts.net/?token=…`) scanned on the hosted Toki RC interface — or a Toki RC link scanned on the Mac's own page — was refused as not being a link for that page, even though both name the same Mac and the manual form accepted them. Either link now connects. A local-network link still can't be used from the hosted interface (an HTTPS page cannot call a plain-HTTP address), but it now says which address it couldn't reach instead of calling the code invalid.
...
- Toki now finds a Claude Code sign-in stored in `~/.claude/.credentials.json`, not just the macOS Keychain, so a Keychain-less install is no longer reported as not connected.
